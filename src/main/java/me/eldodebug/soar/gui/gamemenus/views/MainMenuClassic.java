...
public class MainMenuClassic extends GlideScreen {
    ...
    public MainMenuClassic(MenuManager manager) {
        ...
        addMenuAction(new ViewMenuButton(Icons.IMAGE_EDIT_20, Color.YELLOW, () -> setCurrentView(getViewByClass(BackgroundSelector.class))));

        // Main screen buttons
        buttonSingleplayer = new ElementButton(TranslateText.SINGLEPLAYER, 0, 0, 180, 20, () -> mc.displayGuiScreen(new GuiSelectWorld(getMenuManager())));
        buttonMultiplayer = new ElementButton(TranslateText.MULTIPLAYER, 0, 0, 180, 20, () -> mc.displayGuiScreen(new GuiMultiplayer(getMenuManager())));
        buttonSettings = new ElementButton(TranslateText.SETTINGS, 0, 0, 180, 20, () -> mc.displayGuiScreen(new GuiOptions(getMenuManager(), mc.gameSettings)));
    }
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        buttonSingleplayer.mouseClicked(mouseX, mouseY, mouseButton);
        buttonMultiplayer.mouseClicked(mouseX, mouseY, mouseButton);
        buttonSettings.mouseClicked(mouseX, mouseY, mouseButton);
        super.mouseClicked(mouseX, mouseY, mouseButton);
    }

    @Override
    public void mouseReleased(int mouseX, int mouseY, int mouseButton) {
        buttonSingleplayer.mouseReleased(mouseX, mouseY, mouseButton);
        buttonMultiplayer.mouseReleased(mouseX, mouseY, mouseButton);
        buttonSettings.mouseReleased(mouseX, mouseY, mouseButton);
    }
}